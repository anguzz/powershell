...
function Get-StagedDevices {
  Write-Host "Retrieving managed devices and filtering for null primary user..."
  try {
    $devices = Get-MgDeviceManagementManagedDevice -All

    $filtered = $devices | Where-Object {
      [string]::IsNullOrWhiteSpace($_.userPrincipalName)
    }

    Write-Host "Found $($filtered.Count) devices with no primary user."
    return $filtered
  }
  catch {
...
# ----------------- Main -----------------
$devices = Get-StagedDevices 
if (-not $devices -or $devices.Count -eq 0) {
  Write-Host "No devices found with no primary user. Exiting."
...
Write-Host ("Devices Found   : {0}" -f $stats.Found)
Write-Host ("Skipped         : {0}" -f $stats.Skipped)
Write-Host ("Primary Added   : {0}" -f $stats.Added)
Write-Host ("Primary Replaced: {0}" -f $stats.Replaced)
Write-Host ("No Change       : {0}" -f $stats.NoChange)
