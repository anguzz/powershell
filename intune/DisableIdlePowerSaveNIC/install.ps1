...
$adapters = Get-NetAdapterAdvancedProperty -DisplayName 'Idle Power Saving' | Where-Object {$_.RegistryValue -eq '1'}
foreach ($adapter in $adapters) {
    Write-Log "Found adapter with Idle Power Saving enabled: $($adapter.Name)"
    ...
    Write-Log "Disabled Idle Power Saving on adapter: $($adapter.Name)"
}
